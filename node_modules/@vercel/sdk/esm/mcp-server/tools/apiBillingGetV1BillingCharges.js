/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
import { apiBillingGetV1BillingCharges } from "../../funcs/apiBillingGetV1BillingCharges.js";
import { formatResult } from "../tools.js";
export const tool$apiBillingGetV1BillingCharges = {
    name: "api-billing-get-v1-billing-charges",
    description: `Get FOCUS v1.3 compliant usage cost metrics

Returns usage charges in FOCUS v1.3 JSONL format`,
    tool: async (client, ctx) => {
        const [result, apiCall] = await apiBillingGetV1BillingCharges(client, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {
                content: [{ type: "text", text: result.error.message }],
                isError: true,
            };
        }
        return formatResult(void 0, apiCall);
    },
};
//# sourceMappingURL=apiBillingGetV1BillingCharges.js.map