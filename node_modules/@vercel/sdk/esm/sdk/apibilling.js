/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
import { apiBillingGetV1BillingCharges } from "../funcs/apiBillingGetV1BillingCharges.js";
import { apiBillingGetV1BillingContractCommitments } from "../funcs/apiBillingGetV1BillingContractCommitments.js";
import { ClientSDK } from "../lib/sdks.js";
import { unwrapAsync } from "../types/fp.js";
export class ApiBilling extends ClientSDK {
    /**
     * Get FOCUS v1.3 compliant usage cost metrics
     *
     * @remarks
     * Returns usage charges in FOCUS v1.3 JSONL format
     */
    async getV1BillingCharges(options) {
        return unwrapAsync(apiBillingGetV1BillingCharges(this, options));
    }
    /**
     * Get FOCUS v1.3 compliant contract commitments
     *
     * @remarks
     * Returns contract commitment terms in FOCUS v1.3 JSONL format
     */
    async getV1BillingContractCommitments(options) {
        return unwrapAsync(apiBillingGetV1BillingContractCommitments(this, options));
    }
}
//# sourceMappingURL=apibilling.js.map